...
pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#101e30;color:#e4eefb;padding:14px;border-radius:6px;font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
...
def report(lab, pdf=False, dry=False, open_after=False):
    ...
    def row(event):
        ...
        return ('<tr><td class="t" title="' + escape(stamp(event['time'])) + '">'
                ...
                + escape(lab.redact(str(command))) + '</code></td><td class="s">'
                + escape(str(event.get('duration', '—'))) + '</td><td class="o">'
...
<p class="verdict">{head['verdict']}: {escape(verdict)}</p><p class="muted">{escape(head['lead'])}</p></header>
